...
export async function handler(chatUpdate) {
    ...
    user.role = 'role' in user ? user.role : 'Tadpole';
    user.autolevelup = 'autolevelup' in user ? user.autolevelup : false;
    user.story = 'story' in user ? user.story : true;
} else {
    global.db.data.users[m.sender] = {
        exp: 0,
        registered: false,
  ...
  let games = global.db.data.users[m.sender].games
          if (typeof games !== 'object')
          global.db.data.users[m.sender].games = {}
          if (games) {
          if (!('quiz1' in games)) games.quiz1 = 0
          if (!('quiz2' in games)) games.quiz2 = 0
          if (!('quiz3' in games)) games.quiz3 = 0
          if (!('cap' in games)) games.cap = 0
          if (!('flags' in games)) games.flags = 0
          if (!('qaya' in games)) games.qaya = 0
          if (!('word' in games)) games.word = 0
              } else
          global.db.data.users[m.sender].games = {
          quiz1: 0,
          quiz2: 0,
  ...
  let chat = global.db.data.chats[m.chat]
            ...
            if (chat) {
                ...
                if (!('gptv' in chat)) chat.gptv = false
                if (!('down' in chat))
                  chat.down = true
                if (!('news' in chat))
                    chat.news = false
            ...
            if (settings) {
                ...
                if (!("bloc" in settings)) settings.bloc = false
                 if (!("news" in settings)) settings.news = false
...
return
      ...
      await this.copyNForward(conn.user.id, msg, {
...
global.dfail = (type, m, conn) => {
    const userTag = `👋 وا *@${m.sender.split("@")[0]}*, `
    const emoji = {
        general: '⚙️',
        owner: '👑',
        moderator: '🛡️',
    ...
    const msg = {
        owner: `*${emoji.owner} مول الروبو*\n
    ${userTag} أمر خاص ب *Bot Owner*!`,
        moderator: `*${emoji.moderator} Moderator's Query*\n
    ${userTag} أمر خاص ب *Moderators*!`,
        premium: `*${emoji.premium} Premium Query*\n
    ${userTag} أمر خاص ب *Premium Members*!`,
        group: `*${emoji.group} Group Query*\n
    ${userTag} أمر كايستعمل فقط فـ *Group Chats*!`,
        private: `*${emoji.private} Private Query*\n
    ${userTag} أمر كايستعمل فقط فـ *Private Chats*!`,
        admin: `*${emoji.admin} Admin's Query*\n
    ${userTag} أمر خاص ب *Group Admins*!`,
